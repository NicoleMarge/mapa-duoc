...
# --- 2. INICIALIZACIÓN DE AUTENTICACIÓN (Formato Universal) ---
# Pasamos los parámetros directamente para evitar los errores de 'secret_path'
auth = Authenticate(
...
if not st.session_state.get('connected'):
    ...
    st.markdown("---")
    st.info("Bienvenido. Para acceder, inicia sesión con tu cuenta institucional de Google (@duocuc.cl).")
    
    # Este botón abrirá la ventana de Google para pedir usuario y contraseña
    auth.login()
    st.stop()

# --- 3. VALIDACIÓN DE SEGURIDAD @DUOCUC.CL ---
...
# --- 6. BUSCADOR Y LÓGICA DE MAPAS ---
st.markdown('---')
col_nav, col_busq = st.columns([4, 6])

with col_nav:
    seleccion = st.radio("Edificios:", ["Inicio", "Edificio 1", "Edificio 2", "Edificio 3"], horizontal=True)

with col_busq:
    search_query = st.text_input("Ingresa sala o nombre:", placeholder="Ej: 412, Casino, Biblioteca...")

img_path = None

if search_query and not df.empty:
    query = search_query.strip().upper()
    # Filtrar en las columnas 'sala' o 'nombre' del Excel
    resultado = df[
    ...
    if not resultado.empty:
        ...
        # Selección de imagen por edificio
...
if img_path:
    if os.path.exists(img_path):
        ...
        st.error(f"Falta archivo de imagen: {img_path}")
